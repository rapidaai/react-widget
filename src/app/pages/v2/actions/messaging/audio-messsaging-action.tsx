      ...
      <div className="flex items-center justify-center w-full">
        {!isConnected ? (
          <button
            ...
            className={cn(
              "flex items-center gap-1.5 border-[0.5px] border-primary/10 bg-gray-100 dark:bg-gray-950 rounded-full p-1 shadow-lg  px-4 py-2"
            )}
          >
          ...
          <div className="flex items-center gap-1.5 border-[0.5px] border-primary/10 bg-gray-100 dark:bg-gray-950 rounded-full p-1 shadow-lg ">
            <div className="rounded-full px-2 py-2 flex items-center gap-2">
              ...
              <FlyoutLink
                FlyoutContent={
                  <div className="flex flex-col rounded-lg w-[265px] overflow-hidden bg-white dark:bg-gray-950 border-subtle border-[1px] shadow-lg dark:border-gray-700">
                        ...
                        return (
                          <div
                            ...
                            className={cn(
                              ...
                              "rounded-lg px-2.5 py-2.5 hover:bg-primary/10 hover:text-primary hover:rounded-md text-palette-blue-700 text-sm"
                            )}
                          >
                            <div
              ...
              >
                {(isOpen: boolean) => (
                  <ChevronDown
                    className={cn(
                      "w-5 h-5 transition-transform duration-300 opacity-50 hover:opacity-100 text-gray-800 dark:text-white",
                      isOpen ? "rotate-180" : ""
                    )}
                  />
                )}
            ...
            <Tooltip popupContent="Click for text mode">
              <button
                onClick={async () => {
                  await handleTextToggle(ctx);
                }}
                className="bg-primary/20 backdrop-blur-xl rounded-full shadow-lg p-2 border-[0.2px] border-primary/20 text-primary hover:bg-primary hover:text-white"
              >
                <MessageSquareText className="w-4 h-4" strokeWidth={2} />
              </button>
            </Tooltip>
            <Tooltip popupContent="Click to end session">
              <button
                onClick={async () => {
...
const FlyoutLink: FC<{
  ...
  FlyoutContent: any;
  className: string;
}> = ({ children, FlyoutContent, className }) => {
