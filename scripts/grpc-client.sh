...
# protoc --proto_path="${PROTO_DIR}" --js_out="import_style=commonjs,binary:${OUT_DIR}" --ts_out="service=grpc-web:${OUT_DIR}" "${PROTO_DIR}"/*.proto
 ...
 protoc -I=$PROTO_DIR "${PROTO_DIR}"/*.proto --plugin="protoc-gen-grpc-web=${PROTOC_GEN_TS_PATH}" --js_out=import_style=commonjs,binary:$OUT_DIR  --grpc-web_out=import_style=typescript,mode=grpcweb:$OUT_DIR
